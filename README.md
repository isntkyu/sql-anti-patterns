...
auto_increment (가상 Pk키) 의 빈 번호를 메꾸는 행위를 하지마라.  
행번호랑 헷갈리지 말자.
...
애플리케이션 뿐 아니라 데이터베이스도 문서화, 테스트, 소스관리를 잘하자

---

# 마법의 콩

MVC 모델에서 액티브 레코드 패턴의 단점

- 유닛테스트가 힘들다.
- CRUD를 아무 곳에서나 할 수 있다.(다른 개발자가)
- 도메인 결합도가 약하다.
- 모델과 스키마를 결합시킨 구조이다. (엔티티 클래스와의 결합도가 높음)

장점
- 모델을 DAO와 같이 쓴다.
- 빠르고 편리해서 프로토타입엔 용이하다(꼭 리팩토링 하자)

모델과 액티브레코드(DAO) 의 관계는 상속 (테이블과 일대일) 이 아닌 집합연관이어야 한다.  
